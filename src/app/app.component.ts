...
export class AppComponent {
  ...
  ngOnInit(): void {
    ...
    this.fontStyleControl.valueChanges.subscribe((data) => {
      let errorDetails = this.reportData.find(
        (data) => data.errorMessage === this.fontStyleControl.value
      );
      this.viewData(errorDetails.errorCode);
    });
  }
  constructor(
    private service: DataQualityService,
  ...
  public viewData(errorCode: string) {
    this.service.getViewData(errorCode).subscribe(
      (data) => {
  ...
  value(columnName: string, value: any) {
    return columnName.includes('Date') && value instanceof Date
      ? this.datepipe.transform(value, 'yyyy-MM-dd HH:MM:ss')
      : value;
  }
}
